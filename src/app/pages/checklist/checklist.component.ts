...
export class ChecklistComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.buildForm();
    this.statusSubscription = this.offline.currentStatus.subscribe(
      (isOnline) => {
  ...
  buildForm() {
    this.recordForm = this.fb.group({
      ...
      user_Id: [this.userId],
      isNewRecord: [
        this.params.Record_Id && +this.params.Record_Id > 0 ? false : true,
      ],
  ...
  getChecklistById() {
    this.http
      .get('Checklist/GetChecklistById', { Id: this.id })
      .subscribe(async (value: any) => {
        ...
        this.data = value;
        this.data.submitionState =
          value?.submitionState ?? SubmittionState.SaveAndSubmit;
        ...
        this.options = {
          ...
          renderer: {
            ...
            output: {
              ...
              formLoad: this.onFormReady.bind(this,event)
            },
            input: {
              options: {
                i18n: this.formioI18n,
  ...
  getRecordToEdit() {
    ...
    if (isQrCode) apiUrl = 'ChecklistRecords/CheckIfRecordAssigned';
    this.http
      .get(apiUrl, { Form_Id: this.id, Record_Id: this.params.Record_Id })
      .subscribe((value: any) => {
        if (!value) {
    ...
    if (cashedRecords) {
      ...
      let index = cashedRecords.findIndex((el: any) => {
        return (
          ...
          el.form_Id == this.id &&
          el.record_Id == this.params.Record_Id
        );
      });
  ...
  onFormSubmitted(sender: any, eventData: any) {
    ...
    serializedData = this.helper.adaptFormIoDateTimeValue(
      serializedData,
  ...
  send() {
    ...
    this.modelBody.formDataRef = this.recordForm.get('formDataRef')?.value
      ? this.recordForm.get('formDataRef')?.value
      : this.params.formRef
      ? this.params.formRef
      : null;
    this.statusSubscription2 = this.offline.currentStatus.subscribe(
      async (isOnline) => {
        if (isOnline) {
          this.http
          ...
          let cacheCompletedRecords: any[] =
            (await this.storage.get('CompletedRecords')) || [];
          let recordsWillBeUpserted =
            (await this.storage.get('RecordsWillBeUpserted')) || [];

          if (this.params.offline) {
            let index = cacheRecords.findIndex((el: any) => {
              return (
                el.userId == this.userId && el.offlineRef == this.params.offline
              );
            });
            ...
            if (index >= 0) {
              ...
              let obj = {
                ...
                formDataRef: this.modelBody.formDataRef
                  ? this.modelBody.formDataRef
                  : this.params.formRef,
                form_Id: +this.id,
                ...
                record_Status_Id:
                  this.modelBody.record_Status == RecordStatus.Created
                    ...
                    : this.selectedCashedChecklist?.workflowId
  ...
  async loadChecklistFromCacheById() {
    ...
    if (cacheChecklists.length > 0) {
      value = cacheChecklists.filter(
        (el: any) => el.userId == this.userId && el.formId == this.id
      )[0];
  ...
  async loadRecordFromCacheToEdit() {
    ...
    let valueRecord: any = {};
    if (cacheRecords.length > 0) {
      valueChecklist = cacheChecklists.filter(
        (el: any) => el.userId == this.userId && el.formId == this.id
      )[0];
      this.selectedCashedChecklist = valueChecklist;
      this.selectedCashedChecklist.submitionState =
        valueChecklist?.submitionState ?? SubmittionState.SaveAndSubmit;
      this.submitionState = this.selectedCashedChecklist.submitionState;
      valueRecord = cacheRecords.filter(
        (el: any) =>
          el.userId == this.userId && el.record_Id == this.params.Record_Id
      )[0];

      let requireGps = valueChecklist?.gpsRequired
        ? valueChecklist?.gpsRequired
        : valueRecord?.gpS_Required;
      if (requireGps) {
        navigator.geolocation.getCurrentPosition(
          (location) => {
            this.latitude = location.coords.latitude;
      ...
      if (valueRecord) {
        ...
        let recordJson = valueRecord.record
          ? valueRecord.record
          : valueRecord.record_Json; // data
        ...
        this.form.components = JSON.parse(
          valueChecklist?.formControls
            ? valueChecklist?.formControls
            : valueRecord.form_Layout
        );
        this.options = {
          builder: {
            hideTab: true,
  ...
  ngOnDestroy(): void {
    if (
      this.currentLang &&
      this.formdefaultDisplayLanguage &&
      this.formdefaultDisplayLanguage != this.currentLang
    ) {
  ...
  async updateCashedPlanRecords() {
    ...
    cashedListPlans.forEach((day: any) => {
      day.list.forEach((element: any) => {
        if (
          element.isCreateFormData == true &&
          element.plannerFormsData &&
          element.plannerFormsData[0].formsData
        ) {
          let record = element.plannerFormsData[0].formsData;
          if (
            record.formDataId == +this.params.Record_Id &&
            record.userId == this.userId
          ) {
            element.plannerFormsData[0].formsData.recordStatusId =
              this.modelBody.record_Status;
          }
        }
      });
    });
  ...
  onFormReady(sender:any,event:any) {
    if (this.data?.defaultDisplayLanguage)      
      this.langEmitter.emit(this.data?.defaultDisplayLanguage);
  }
}
